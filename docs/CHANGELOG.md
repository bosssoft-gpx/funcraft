...
- **Hook: `useEvent`**
  - 用于创建一个具有稳定引用的事件处理函数，避免闭包陷阱和不必要的重新绑定。

### 📝 文档
- 新增 **`useEvent`** 使用文档：API、参数说明、示例、注意事项。
...
- **Function: `isPostalCode`**
  - 验证字符串是否为有效的邮政编码，暂仅支持中国大陆邮编格式（6位数字，首位非0）

### 📝 文档
- 新增 **`isPostalCode`** 使用文档

### ✅ 测试
- 新增 **Vitest** 单元测试覆盖 `isPostalCode` 的各种输入类型与边界情况
...
- **Function: `safeErrorWrapper`**
  - 将任意类型的错误包装成 Error 对象，确保一致的错误处理体验

### 📝 文档
- 新增 **`safeErrorWrapper`** 使用文档：API、参数说明、示例、注意事项。

### ✅ 测试
- 新增 **Vitest** 单元测试覆盖 `safeErrorWrapper` 的各种输入类型与边界情况
---
